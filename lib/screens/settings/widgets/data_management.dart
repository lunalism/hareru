...
Future<void> backupData(BuildContext context) async {
  ...
  // Warn user that backup contains transaction data
  ...
  final settingsBox = await HiveEncryptionService.openBox<dynamic>('settings');
  final transactionsBox = await HiveEncryptionService.openBox<Transaction>('transactions');
  final categoriesBox = await HiveEncryptionService.openBox<cat.Category>('categories');
...
Future<void> restoreData(BuildContext context, WidgetRef ref) async {
  ...
  try {
    ...
    if (bytes.length > 5 * 1024 * 1024) return; // 5MB limit

    final jsonStr = utf8.decode(bytes);
    final data = json.decode(jsonStr) as Map<String, dynamic>;

    if (data['transactions'] is! List) return;
    ...
    if (data['transactions'] is List) {
      final transactionsBox =
          await HiveEncryptionService.openBox<Transaction>('transactions');
